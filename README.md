...
| 구분      | 작업                             | 기술                                    | 비고                            |
|-----------|----------------------------------|-----------------------------------------|---------------------------------|
| 서버      | 1분 간격 호출                     | 서버 스케줄러 (e.g. Cron)             | 시간 간격 변경 가능             |
|           | 스톡 생성 (하루 분량)            |                                         | 자정 초기화, n/5 간격 실행      |
| 로그인    | 과수원 이름 (ID)                  |                                         | pldata테이블                       |
|           | 폰번호 (PW, 범위 010-0000-0000)  | 패턴 매칭 정규식                        | "-" 제거 후 해싱 저장           |
| 해싱      | 단순해시 사용 (SHA-256)            | 컴퓨터 해싱 알고리즘(e.g. crypto-js)    |                                 |
| 실시간    | 차팅 기능                         | 서버와 클라이언트 통신 (WebSockets)    | AJAX 사용은 보류                |
| 클라이언트 | 스톡 데이터 받아서 배열에 저장    | 클라이언트 스크립트 with 필요한 데이터 | 렌더링 중 스톡 데이터 업데이트 |
|           | 서버에서 값 호출 및 스톡 생성 주기 | 서버와 클라이언트 통신                | 예시 - 3초, 12초, 15초          |


+---------------------+     +-------------------+     +--------------------+
| 데이터베이스에서 값 가져오기 | --> | JSON 배열에 날짜 순으로 저장 | --> | 초기 차트 그리기  |
+---------------------+     +-------------------+     +--------------------+
           ^                                                  |
           |                                                  v
    +--------------+                                  +---------------+
    ...
    +--------------+                                  +---------------+
           ...
           v                                                  v
  +------------------+                             +----------------------+
  | 현재 온도 및 최저/최고 온도 표시   |--->| 특정 시간대별 최저/최고 온도 기록 및 표시|
  +------------------+                             +----------------------+
